...
class EmailInputViewController: UIViewController {
    ...
    func configureBindings() {
        let emailObservable = usernameTextField.rx.text.orEmpty
            .map { [weak self] text in
                guard let self = self else { return "" }
                return "\(text)@\(self.selectedDomain)"
            }
            .share(replay: 1)
        ...
        emailObservable
            .map { [weak self] email in
                guard let self = self else { return UIColor.lightGray }
                return self.isValidEmail(email) ? CustomColors.softPink : UIColor.lightGray
            }
            .bind(to: duplicateCheckButton.rx.backgroundColor)
            .disposed(by: disposeBag)

        duplicateCheckButton.rx.tap
            .flatMapLatest { [weak self] _ -> Observable<Bool> in
                guard let self = self else { return Observable.just(false) }
                return self.checkEmailDuplicate()
            ...
            .subscribe(onNext: { [weak self] isValid in
                ...
                self.duplicateCheckButton.setTitle(isValid ? "사용 가능" : "이미 사용 중", for: .normal)
                self.duplicateCheckButton.backgroundColor = isValid ? UIColor.green : UIColor.red
                self.nextButton.isEnabled = isValid
                self.nextButton.backgroundColor = isValid ? CustomColors.softPink : UIColor.lightGray
            })
            .disposed(by: disposeBag)
        ...
        domainButton.rx.tap
            .subscribe(onNext: { [weak self] in
                self?.showDomainSelection()
            })
            .disposed(by: disposeBag)
    }
    
    func checkEmailDuplicate() -> Observable<Bool> {
        return Observable<Bool>.create { [weak self] observer in
            ...
            let email = self.getEmailAddress()
    ...
    func nextButtonTapped() {
        ...
        if isEmailValid {
            ...
            passwordInputVC.nickname = nickname
            passwordInputVC.email = email
